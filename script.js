...
(function () {
    document.querySelector('#uuid').textContent = crypto.randomUUID();


    document.querySelector('#copy').addEventListener('click', function () {
        var copyText = document.querySelector('#uuid').textContent;
        navigator.clipboard.writeText(copyText).then(function () {
            NotificationManager.create('Copied!', 'UUID copied to clipboard', 2);
        }, function (err) {
            NotificationManager.create('Error!', 'Failed to copy UUID to clipboard', 2);
        });
    });
})();